...
class AttemptTest < Minitest::Test
  ...
  def setup
    @user = User.create(current: {'nong' => 'two', 'femp' => 'two'})
  ...
  def test_saving_an_attempt_constructs_a_submission
    ...
    Attempt.new(user, 'CODE', 'two.no', curriculum).save
  ...
  def test_attempt_is_created_for_correct_submission
    ...
    Attempt.new(user, 'CODE', 'two.fp', curriculum).save
  ...
  def test_a_new_attempt_supersedes_the_previous_one
    Attempt.new(user, 'CODE 1', 'two.fp', curriculum).save
    Attempt.new(user, 'CODE 2', 'two.fp', curriculum).save
  ...
  def test_previous_submission_after_first_attempt_in_new_language
    Attempt.new(user, 'CODE 1', 'two.fp', curriculum).save
    attempt = Attempt.new(user, 'CODE 2', 'two.no', curriculum).save
    assert_equal attempt.previous_submission.language, "nong"
  end

  def test_previous_submission_after_superseding
    Attempt.new(user, 'CODE 1', 'two.fp', curriculum).save
    attempt = Attempt.new(user, 'CODE 2', 'two.fp', curriculum).save
  ...
  def test_previous_submission_with_new_language_sandwich
    Attempt.new(user, 'CODE 1', 'two.fp', curriculum).save
    Attempt.new(user, 'CODE 2', 'two.no', curriculum).save
    attempt = Attempt.new(user, 'CODE 3', 'two.fp', curriculum).save
