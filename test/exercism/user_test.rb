...
class UserTest < Minitest::Test
  ...
  def github_user(id, username, email, avatar_url)
    GitHubUser.new("id" => id, "login" => username, "email" => email, "avatar_url" => avatar_url)
  end

  def test_create_user_from_github
    github_user = github_user(23, 'alice', 'alice@example.com', 'avatar_url')
    user = User.from_authentication_data(github_user, 'polyglot')
  ...
  def test_update_username_from_github
    ...
    github_user = github_user(23, 'bob', nil, nil)
    user = User.from_authentication_data(github_user).reload
  ...
  def test_does_not_overwrite_email_or_joined_as_if_present
    ...
    github_user = github_user(23, nil, 'new@example.com', nil)
    user = User.from_authentication_data(github_user, 'artisan').reload
    assert_equal 'alice@example.com', user.email
    assert_equal 'polyglot', user.joined_as
  end
  ...
  def test_sets_avatar_url
    User.create(github_id: 23)
    github_user = github_user(23, nil, nil, 'new?1234')
    user = User.from_authentication_data(github_user).reload
  ...
  def test_from_github_unsets_old_duplicate_username
    ...
    u2 = User.from_authentication_data(github_user(31, 'alice', nil, nil)).reload
    assert_equal 'alice', u2.username
    assert_equal '', u1.reload.username

    # it doesn't overwrite it's own username next time
    u3 = User.from_authentication_data(github_user(31, 'alice', nil, nil)).reload
    assert_equal 'alice', u3.username
  end

  def test_from_github_connects_invited_user
    u1 = User.create(username: 'alice')
    github_user = github_user(42, 'alice', 'alice@example.com', 'avatar')
    u2 = User.from_authentication_data(github_user).reload
